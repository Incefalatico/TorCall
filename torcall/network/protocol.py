...
# over the ephemeral key.  Four wire forms are accepted for backward
# compatibility:
#
...
#                                  + freshness nonce(16).
#   * 146+N bytes           → v3: v2 layout + addr_len(2) + onion addr(N).
#
# The v3 form lets the sender advertise its own ``.onion`` address inside
# the handshake.  This matters for *incoming* calls: the connection is
# forwarded from the local Tor daemon, so the receiving socket only ever
# sees ``127.0.0.1`` and has no other way to learn who is calling.  The
# advertised address is bound into the v3 signature (see crypto.py) so it
...
_SIGNATURE_LEN = 64
_NONCE_LEN = 16
_ADDR_LEN_FIELD = 2  # big-endian uint16 length prefix for the onion address
_HANDSHAKE_FULL_LEN = _X25519_LEN + _ED25519_LEN + _SIGNATURE_LEN  # 128
_HANDSHAKE_V2_LEN = _HANDSHAKE_FULL_LEN + _NONCE_LEN  # 144
_HANDSHAKE_V3_MIN_LEN = _HANDSHAKE_V2_LEN + _ADDR_LEN_FIELD  # 146 (empty addr)
...
def encode_handshake(
    ...
    (144-byte) form, otherwise the v1 (128-byte) form.  Supplying
    *onion_address* (which requires *nonce*) yields the v3 form, where the
    sender advertises its own ``.onion`` address so the peer can pin it
    even when the socket only shows a loopback address.  With neither
    ...
    if onion_address is None:
        return base
    addr_bytes = onion_address.encode("utf-8")
    if len(addr_bytes) > 0xFFFF:
        raise ValueError("Onion address too long to encode")
    return base + struct.pack(">H", len(addr_bytes)) + addr_bytes


def decode_handshake(
    payload: bytes,
) -> tuple[bytes, Optional[bytes], Optional[bytes], Optional[bytes], Optional[str]]:
    """Parse a handshake payload.

    Returns ``(ephemeral_public, identity_public, signature, nonce,
    onion_address)`` where trailing fields are ``None`` for shorter forms
    (a legacy 32-byte payload yields everything past the ephemeral key as
    None; a v1 128-byte payload yields nonce/address = None; a v2 144-byte
    payload yields address = None).
    ...
    if len(payload) == _HANDSHAKE_FULL_LEN:
        ...
        return ephemeral, identity, signature, None, None
    if len(payload) == _HANDSHAKE_V2_LEN:
        ...
        return ephemeral, identity, signature, nonce, None
    if len(payload) >= _HANDSHAKE_V3_MIN_LEN:
        ephemeral = payload[:_X25519_LEN]
        identity = payload[_X25519_LEN : _X25519_LEN + _ED25519_LEN]
        signature = payload[_X25519_LEN + _ED25519_LEN : _HANDSHAKE_FULL_LEN]
        nonce = payload[_HANDSHAKE_FULL_LEN : _HANDSHAKE_V2_LEN]
        (addr_len,) = struct.unpack(
            ">H", payload[_HANDSHAKE_V2_LEN : _HANDSHAKE_V3_MIN_LEN]
        )
        addr_bytes = payload[_HANDSHAKE_V3_MIN_LEN : _HANDSHAKE_V3_MIN_LEN + addr_len]
        if len(addr_bytes) != addr_len:
            raise ValueError("Handshake onion address truncated")
        try:
            onion_address = addr_bytes.decode("utf-8")
        except UnicodeDecodeError as exc:
            raise ValueError("Handshake onion address is not valid UTF-8") from exc
        return ephemeral, identity, signature, nonce, onion_address
