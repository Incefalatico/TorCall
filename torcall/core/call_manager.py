...
class CallManager(QObject):
    ...
    def _build_handshake(self) -> bytes:
        ...
        signature = sign_handshake(self._identity.private_key, self._my_public, nonce)
    ...
    def _verify_peer_identity(self, payload: bytes, address: str) -> Optional[bytes]:
        ...
        """
        try:
            ephemeral, identity_pub, signature, nonce = decode_handshake(payload)
        except ValueError:
        ...
        if not verify_handshake(identity_pub, ephemeral, signature, nonce):
        ...
        status = self._contacts.check(address, identity_pub)
        self._pending_peer_identity = {
            "status": status,
            "fingerprint": fingerprint(identity_pub),
            "address": address,
